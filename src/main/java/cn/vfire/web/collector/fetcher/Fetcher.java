...
public class Fetcher {
			...
			try {
				...
				while (running) {
					try {
						 ...
						 * "Exception when updating db", ex); } } if
						 * (visitInterval > 0) { try {
	 ...
	 * 
	 * try { visitor.visit(page, next); } catch (Exception ex) { LOG.info(
	 * "Exception when visit URL: " + url, ex); return false; } } else { try {
	 * visitor.notFound(page, next); } catch (Exception ex) { LOG.info(
	 * "Exception when not found URL: " + url, ex); return false; } }
	 * 
	 * try { visitor.afterVisit(page, next); } catch (Exception ex) { LOG.info(
	 * "Exception after visit URL: " + url, ex); return false; }
	 * 
	 * } else if (page.getStatus() == Page.STATUS_FETCH_FAILED) {
	 * 
	 * crawlDatum.setStatus(CrawlDatum.STATUS_DB_UNFETCHED);
	 * 
	 * try { visitor.fail(page, next); } catch (Exception ex) { LOG.info(
	 * "Exception when execute failed URL: " + url, ex); return false; } }
	 * return true; }
	 * 
	 * public Page getPage(CrawlDatum crawlDatum) {
	 * 
	 * String url = crawlDatum.getUrl(); Page page; HttpResponse response =
	 * null; int retryIndex = 0; Exception lastException = null; int retryCount
	 * = 0; for (; retryIndex <= retry; retryIndex++) { try { response =
	 * requester.getResponse(crawlDatum);//this.getHttpResponse(crawlDatum);
	 * break; } catch (Exception ex) {
	 * 
	 * String suffix = "th "; switch (retryIndex + 1) { case 1: suffix = "st ";
	 * break; case 2: suffix = "nd "; break; case 3: suffix = "rd "; break;
	 * default: suffix = "th "; }
	 * 
	 * lastException = ex;
	 * 
	 ...
	 * .append(url).append(" after ").append(retryInterval) .append("ms "
	 * ).append("(").append(ex.toString()).append(")"); String logMessage =
	 * sb.toString(); LOG.info(logMessage); retryCount++; if (retryInterval > 0)
	 * { try { Thread.sleep(retryInterval); } catch (Exception sleepEx) { } } }
	 * 
	 * } }
	 * 
	 * if (response != null) { if (!response.isNotFound()) { LOG.info(
	 * "fetch URL: " + url); } else { //404应该被当作抓取成功，因为404告诉爬虫页面不存在，以后不需要重试页面
	 * LOG.info("ignore URL: " + url + " (not found)"); } page =
	 * Page.createSuccessPage(crawlDatum, retryCount, response);
	 * 
	 * } else { LOG.info("failed URL: " + url + " (" + lastException + ")");
	 ...
	 * 
	 * public void setRetryInterval(long retryInterval) { this.retryInterval =
	 * retryInterval; }
	 * 
	 * public long getVisitInterval() { return visitInterval; }
	 * 
	 * public void setVisitInterval(long visitInterval) { this.visitInterval =
	 * visitInterval; }
	 */
