...
import cn.vfire.web.collector.plugin.ram.RamCrawler;

import org.jsoup.nodes.Element;

/**
 * 本教程演示了WebCollector 2.20的新特性: 1)MetaData:
 ...
 * 
 * 2)RamCrawler: RamCrawler不需要依赖文件系统或数据库，适合一次性的爬取任务.
...
public class DemoBingCrawler extends RamCrawler {
	...
	public DemoBingCrawler(String keyword, int maxPageNum) throws Exception {
		for (int pageNum = 1; pageNum <= maxPageNum; pageNum++) {
			...
			CrawlDatum datum = new CrawlDatum(url).meta("keyword", keyword).meta("pageNum", pageNum + "")
					.meta("pageType", "searchEngine").meta("depth", "1");
			addSeed(datum);
		}
	}
	...
	public void visit(Page page, CrawlDatums next) {
		...
		if (pageType.equals("searchEngine")) {
			...
			for (int rank = 0; rank < results.size(); rank++) {
				 ...
				 */
				CrawlDatum datum = new CrawlDatum(result.attr("abs:href")).meta("keyword", keyword)
						.meta("pageNum", pageNum + "").meta("rank", rank + "").meta("pageType", "outlink")
			...
			String line = String.format("第%s页第%s个结果:%s(%s字节)\tdepth=%s\treferer=%s", pageNum, rank + 1,
					page.doc().title(), page.getContent().length, depth, referer);
