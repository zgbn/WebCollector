...
public class RedisSegmentWriter implements SegmentWriter {
	...
	public RedisSegmentWriter(RedisDBConfig.DBName dbName) {
		this.dbName = dbName;
	}
	...
	public void wrtieFetchSegment(CrawlDatum fetchDatum) throws CrawlerDBException {
		...
		boolean invalid = fetchDatum.isInvalid();
		...
		try {
			...
			if (invalid) {
				pipeline.lpush(this.dbName.queueInvalidKey, value);
			} else {
				pipeline.lpush(this.dbName.queueKey, value);
			}
